...
`fiber` 节点就是描述随后要执行的工作的一种数据结构，可以类比于调用栈中的帧。
`fiber` 架构同时还提供了便捷的方式去**追踪**，**调度**，**暂停**，**中断 reconcile 过程**
...
并且 Lanes 模型分配了一组 lanes，每个 lane 都有不同的优先级，React 可以根据不同的优先级来分配时间去更新应用的 UI，这样就可以更灵活地处理复杂的 UI 更新情况，并且可以提高应用的性能。
