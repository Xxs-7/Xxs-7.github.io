...
链接：https://juejin.cn/post/6844903471162851342
```
...
3. 破坏了 PureComponent、React.memo 浅⽐较的性能优化效果（为了获取最新的 props 和 state，每次 render() 都要重新

创建事件处理函数）

1. 在闭包场景中可能会引⽤到旧的 state、props 值
内部实现上不直观（依赖⼀份可变的全局状态，不再那么“纯”）
2. React.memo 并不能完全替代 shouldComponentUpdate（因为获取不到 state 的变化，只针对 props 的变化）